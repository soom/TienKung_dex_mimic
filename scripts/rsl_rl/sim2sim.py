"""Standalone Mujoco sim2sim runner for Tienkung2 Pro ONNX motion policies.
...
DEFAULT_POLICY = Path(
	"logs/rsl_rl/tienkung2_pro_fix/0409_130934_teleop_teacher_full/exported/policy.onnx"
)
...
def infer_total_steps(onnx_model: onnx.ModelProto) -> int:
	for node in onnx_model.graph.node:
		if node.op_type == "Constant" and "/Constant_output_0" in node.output:
			for attr in node.attribute:
				if attr.name == "value":
					value = onnx.numpy_helper.to_array(attr.t)
					return int(float(value)) + 1
	raise RuntimeError("Failed to infer motion length from ONNX graph.")
...
class OnnxMotionPolicy:
	def __init__(self, policy_path: Path):
		...
		self.input_obs_name = self.session.get_inputs()[0].name
		self.input_step_name = self.session.get_inputs()[1].name
	...
	def infer(self, obs: np.ndarray, time_step: int) -> dict[str, np.ndarray]:
		outputs = self.session.run(
			self.output_names,
			{
				self.input_obs_name: obs.reshape(1, -1).astype(np.float32),
				self.input_step_name: np.array([[time_step]], dtype=np.float32),
			},
		)
...
class NpzTeleopStream:
	...
	Each call to next_frame() returns:
	  - q_ref_full  : (27,) full joint reference — legs from NPZ, arms from IK
	  - qd_ref_full : (27,) joint velocity reference — legs from NPZ, arms zeroed
	...
	def __init__(
		...
		if "body_names" not in data.files:
			raise RuntimeError(
				...
				"Please regenerate it with the latest scripts/csv_to_npz.py."
			)
...
FOOT_GEOM_NAMES = ["toe1_left", "toe2_left", "toe1_right", "toe2_right"]
...
class MujocoSim2Sim:
	def __init__(
		...
		return_transition_duration_s: float = 1.0,
	):
		...
		self.transition_duration_s = max(float(transition_duration_s), self.control_dt)
		self.return_transition_duration_s = max(float(return_transition_duration_s), self.control_dt)
		self._request_lock = threading.Lock()
		self._pending_next_motion_requests = 0
		self._playback_phase = "motion_playback"
		self._transition: BlendTransition | None = None
		...
		# Idle stream: used to run the policy during default_pose phase so the robot
		# has a valid static reference to track (instead of an untrained hold task).
		self._idle_stream: NpzTeleopStream | None = None
		self._idle_step: int = 0
		if self.motion_library is not None:
			idle_idx = self.motion_library.find_idle_index()
			if idle_idx is not None:
				self._idle_stream = self.motion_library.get_stream(idle_idx)
				print(f"[playlist] idle stream: {self.motion_library.motion_names[idle_idx]}")
			else:
				print("[playlist] WARNING: no init_pose NPZ found, default_pose will use PD only")
	...
	def _apply_policy_dynamics(self) -> None:
		for joint_index, joint_name in enumerate(self.metadata.joint_names):
			...
			effort_limit = max(float(self.metadata.action_scale[joint_index] * kp / 0.1875), kp * 0.5)
	...
	def _resolve_body_id(self, body_name: str) -> int:
		...
		alias_name = body_aliases.get(body_name)
		if alias_name is None:
			return -1
		return mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_BODY, alias_name)
	...
	def _foot_contact_forces(self) -> tuple[float, float]:
		...
		left_geoms = {
			...
			for n in ["toe1_left", "toe2_left"]
		}
		right_geoms = {
			mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_GEOM, n)
			for n in ["toe1_right", "toe2_right"]
		}
		for i in range(self.data.ncon):
			c = self.data.contact[i]
			g1, g2 = int(c.geom1), int(c.geom2)
			pair = {g1, g2}
			# contact force in contact frame: data.contact[i].frame, use mj_contactForce
			cf = np.zeros(6)
	...
	def _foot_xy_vel(self) -> tuple[float, float]:
		...
		for bid in self._foot_body_ids:
			# cvel shape: (nbody, 6) — [ang(3), lin(3)] in world frame
	...
	def _set_reference_frame_from_robot(self, ref0: "dict[str, np.ndarray] | None" = None) -> None:
		...
		If ref0 is provided (the first frame of the incoming NPZ), the yaw offset
		is computed as (robot_yaw - ref0_yaw) so that after _normalize_reference_output
		the anchor orientation in the reference matches the robot's current heading.
		The XY offset is set to ref0's pelvis XY so that the reference origin lands
		at the robot's current XY position.

		If ref0 is None (e.g. entering idle/default phase), only the robot's current
		pose is used to zero out any accumulated offset.
		"""
		if not self.zero_xy_ref:
			self._reference_xy_offset[:] = 0.0
			self._reference_yaw = 0.0
		...
		if ref0 is not None:
			# XY offset: translate ref so its origin lands at robot's current XY
			ref_xy = np.asarray(ref0["body_pos_w"][0, :2], dtype=np.float64)
			self._reference_xy_offset = ref_xy.copy()
			# Yaw offset: rotate ref so its heading matches robot's current heading
			ref_yaw = yaw_from_quat(ref0["body_quat_w"][0])
			delta_yaw = robot_yaw - ref_yaw
		else:
			# Idle/default: align reference to robot's current XY and heading.
			self._reference_xy_offset = np.array(robot_pos[:2], dtype=np.float64)
			delta_yaw = robot_yaw  # rotate idle/default reference to match robot's current yaw
	...
	def _build_default_reference(self) -> dict[str, np.ndarray]:
		if not self.reference_outputs:
			raise RuntimeError("Default reference requires ONNX reference outputs.")
		ref0 = self.reference_outputs[0]
		temp_data = mujoco.MjData(self.model)
		# Use identity orientation (upright) so anchor_rot6d is zero-error when robot stands straight
		base_height = float(ref0["body_pos_w"][0, 2])
	...
	def _run_policy_step(
		...
		ref_now / ref_lookahead: used for the PD joint target (q_target = q_ref + blend_alpha * action * scale).
		obs_ref / obs_ref_lookahead: if provided, used for q_ref / qd_ref / anchor_rot6d in the observation.
		  Keeps the policy in-distribution during blend transitions.
		blend_alpha: scales the policy action contribution (0 = pure reference tracking, 1 = full policy).
		"""
	...
	def _start_motion_transition(self) -> None:
		if self.motion_library is None:
			return
		next_index = 0 if self._active_motion_index is None else (self._active_motion_index + 1) % self.motion_library.motion_count
		self.teleop_stream = self.motion_library.get_stream(next_index)
		self.teleop_stream.reset()
		# Align reference frame: XY origin → ref frame0 XY, yaw → (robot_yaw - ref_yaw)
		# so that after normalization the anchor heading matches the robot's current heading.
		...
		# start_frame: robot's current state in world coords (NOT normalized).
		# body_quat_w[0] = robot's actual quat → anchor_rot6d = identity at alpha=0.
		# end_frame: ref0 normalized to robot's current XY/yaw.
		# body_quat_w[0] = quat_from_yaw(robot_yaw) → anchor_rot6d = identity at alpha=1.
		# _aligned_reference_bodies uses only relative positions, so mixed coords are fine.
		start_frame = self._build_current_robot_frame()
		end_frame = self._normalize_reference_output(ref0_raw)
		self._transition = BlendTransition(
			start_frame=start_frame,
			end_frame=end_frame,
			total_steps=self._transition_steps(self.transition_duration_s),
		)
		self._playback_phase = "transition_to_motion"
		self.current_step = 0
		self.last_action[:] = 0.0
		print(
			f"[playlist] switching to motion {next_index + 1}/{self.motion_library.motion_count}: "
			f"{self._active_motion_name}"
		)

	def _start_return_transition(self) -> None:
		# Align reference frame to robot's current pose so end_frame body positions
		# land at the robot's current XY location, not at the recording origin.
		self._set_reference_frame_from_robot()
		# start_frame: robot's current state (world coords, NOT normalized).
		# body_quat_w[0] = robot's actual quat → anchor_rot6d = identity at alpha=0.
		# end_frame: default_reference normalized to robot's current XY/yaw.
		start_frame = self._build_current_robot_frame()
		end_frame = self._normalize_reference_output(self.default_reference)
		self._transition = BlendTransition(
			start_frame=start_frame,
			end_frame=end_frame,
			total_steps=self._transition_steps(self.return_transition_duration_s),
		)
		self._playback_phase = "transition_to_default"
		self.current_step = 0
		print("[playlist] blending back to default_joint_pos flow")

	def _step_motion_playlist(self) -> bool:
		if self._consume_next_motion_request():
			self._start_motion_transition()

		if self._playback_phase == "default_pose":
			if self._idle_stream is not None:
				ref_now = self._normalize_reference_output(self._idle_stream.reference_at(self._idle_step))
				lookahead_step = min(self._idle_step + LOOKAHEAD_STEPS, self._idle_stream.total_steps - 1)
				ref_lookahead = self._normalize_reference_output(self._idle_stream.reference_at(lookahead_step))
				total = self._idle_stream.total_steps
				self._run_policy_step(ref_now, ref_lookahead, self._idle_step % total,
				                      override_phase=0.0)
				self._idle_step = (self._idle_step + 1) % total
			else:
				self._apply_joint_target(self.default_reference["joint_pos"])
				self._step_physics()
				self.last_action[:] = 0.0
			return False

		if self._playback_phase == "transition_to_motion":
			tr = self._transition
			if tr is None:
				self._playback_phase = "play_motion"
				return False
			raw_alpha = (tr.step + 1) / tr.total_steps
			alpha = smoothstep(min(raw_alpha, 1.0))
			lookahead_alpha = smoothstep(min(raw_alpha + LOOKAHEAD_STEPS / tr.total_steps, 1.0))
			ref_now = self._interpolate_reference_frames(tr.start_frame, tr.end_frame, alpha)
			ref_lookahead = self._interpolate_reference_frames(tr.start_frame, tr.end_frame, lookahead_alpha)
			# Policy obs uses end_frame (NPZ frame0) as q_ref — keeps policy in-distribution.
			# Target uses interpolated ref_now with blend_alpha=alpha (0→1) for smooth ramp-up.
			self._run_policy_step(ref_now, ref_lookahead, 0,
			                      blend_alpha=alpha,
			                      obs_ref=tr.end_frame,
			                      obs_ref_lookahead=tr.end_frame)
			tr.step += 1
			self.current_step = tr.step
			if tr.step >= tr.total_steps:
		...
		if self._playback_phase == "transition_to_default":
			tr = self._transition
			if tr is None:
			...
			ref_now = self._interpolate_reference_frames(tr.start_frame, tr.end_frame, alpha)
			ref_lookahead = self._interpolate_reference_frames(tr.start_frame, tr.end_frame, lookahead_alpha)
			# Policy obs uses end_frame (default reference, ≈ init_pose frame0) as q_ref.
			# Target uses interpolated ref_now with blend_alpha=(1-alpha) for smooth ramp-down.
			self._run_policy_step(ref_now, ref_lookahead, 0,
			                      blend_alpha=1.0 - alpha,
			                      obs_ref=tr.end_frame,
			                      obs_ref_lookahead=tr.end_frame)
			tr.step += 1
			self.current_step = tr.step
			if tr.step >= tr.total_steps:
				...
				self._set_reference_frame_from_robot()
				self._idle_step = 0
				print("[playlist] back on default_joint_pos flow, press B for next motion")
			return False
	...
	def reset(self) -> None:
		...
		if self.motion_library is not None:
			...
			self._transition = None
			self._idle_step = 0
			# Reset reference frame to origin: robot is placed at [0,0,height] identity on reset.
			self._reference_xy_offset = np.zeros(2, dtype=np.float64)
			self._reference_yaw = 0.0
			self._reference_yaw_inv = np.array([1.0, 0.0, 0.0, 0.0], dtype=np.float64)
	...
	def build_observation(
		...
		obs_ref / obs_ref_lookahead: if provided, these are used for q_ref / qd_ref /
		anchor_rot6d in the observation, while ref_now is still used for the joint target
		computation in _run_policy_step.  This lets transitions keep the policy
		in-distribution (obs sees the stable NPZ reference) while the PD target smoothly
		...
		# Use obs_ref if provided (transition mode), otherwise fall back to ref_now
...
def build_argparser() -> argparse.ArgumentParser:
	parser = argparse.ArgumentParser(description="Standalone Mujoco sim2sim runner for Tienkung2 Pro ONNX policy.")
	...
	parser.add_argument(
		...
		type=float,
		default=1.0,
		help="Blend duration from default_joint_pos to a motion start when using --npz-dir.",
	)
	parser.add_argument(
		"--return-transition-seconds",
		type=float,
		default=1.0,
		help="Blend duration from a motion end back to default_joint_pos when using --npz-dir.",
	)
	return parser
...
def run_with_viewer(sim: MujocoSim2Sim, max_steps: int | None, loop: bool, print_every: int, playback_rate: float) -> None:
	...
	with mujoco_viewer.launch_passive(sim.model, sim.data, key_callback=on_key) as viewer:
		step_count = 0
...
def main() -> None:
	...
	sim = MujocoSim2Sim(
		...
		transition_duration_s=args.transition_seconds,
		return_transition_duration_s=args.return_transition_seconds,
	)
