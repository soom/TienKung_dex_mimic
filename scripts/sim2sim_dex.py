"""MuJoCo runner for Dex EVT ONNX policies and NPZ motion playlists."""
...
class MujocoSim2Sim:
    ...
    def __init__(
        ...
        # Keep the standing reference consistent with the policy and NPZ
        # transition pose.  The standing head was trained around this offset.
    ...
    def _apply_policy_dynamics(self) -> None:
        # Configure position-style PD actuators so ctrl is a joint target
    ...
    def _build_current_robot_frame(self) -> dict[str, np.ndarray]:
        ...
        for body_name in self.metadata.body_names:
            ...
            body_pos_w.append(np.asarray(self.data.xpos[body_id], dtype=np.float64).copy())
            body_quat_w.append(quat_normalize(self.data.xquat[body_id]))
        body_pos_w = np.stack(body_pos_w)
        return {
            "joint_pos": self._current_joint_positions(),
            "joint_vel": self.data.qvel[self.joint_qvel_indices].astype(np.float32, copy=True),
            "body_pos_w": body_pos_w,
    ...
    def _slerp_wxyz(qa: np.ndarray, qb: np.ndarray, alpha: float) -> np.ndarray:
        ...
        return quat_normalize(
            np.sin((1.0 - alpha) * theta) / s * qa
            + np.sin(alpha * theta) / s * qb
        )
    ...
    def _step_motion_playlist(self) -> bool:
        ...
        if self._playback_phase == self.PLAY_MOTION:
            ...
            # Dex transition clips already contain a stationary tail at the
            # training default pose.  Keep the tracking head active through
            # that tail; cross-fading early to the standing head introduces an
            # unnecessary backward/side step.
            tracking_weight = fade_in
            ...
            if self.current_step >= self._total_steps - 1:
                ...
                # `last_action` is part of the observation, and the EMA target
                # otherwise retains the final tracking residual after the head
                # switch, producing a large recovery step.
