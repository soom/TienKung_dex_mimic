...
def set_episode_length_from_motion_file(env_cfg: "TrackingEnvCfg", motion_file: str) -> float:
    motion_path = Path(motion_file).expanduser().resolve()
    data = np.load(motion_path)
    episode_length_s = int(data["joint_pos"].shape[0]) / float(data["fps"].item())
...
class EventCfg:
    ...
    # Ankle joints have larger calibration errors in practice; use wider offset range.
    ...
    # interval
    # Paper Table S2: interval U[1.0, 3.0]s, lin_vel ±0.5 m/s, yaw ±0.78 rad/s.
