#!/usr/bin/env bash
# Walker C1 Simple 动作跟踪训练脚本
# 用法：bash train_c1.sh [smoke|medium|full] [motion_npz_or_dir] [num_gpus] [resume_run] [checkpoint]
#
# 示例：
#   bash train_c1.sh smoke                              # 冒烟测试（256 envs, 500 iter）
#   bash train_c1.sh medium                             # 中等规模（1024 envs, 5000 iter）
#   bash train_c1.sh full                               # 全量训练（默认 dataset/npz_c1）
#   bash train_c1.sh full motions/c1_motion.npz 2       # 显式传 motion + GPU 数
#   RESUME=true bash train_c1.sh full motions/c1_motion.npz 2 run_name model_500.pt
#   RESUME=false LOAD_CKPT=policy/c1/wbt_model_26600.pt bash train_c1.sh full
#   TASK_ID=Tracking-Flat-WalkerC1-Simple-v0 bash train_c1.sh full motions/c1_motion.npz 2
#   EPISODE_LENGTH_CAP_S=12 bash train_c1.sh full motions/c1_motion.npz 2
#
# 说明：
#   - 任务是 `Tracking-Flat-WalkerC1-Simple-v0`
...
DEFAULT_MOTION_FILE="dataset/npz_c1"

MODE="${1:-full}"
MOTION_FILE="${2:-$DEFAULT_MOTION_FILE}"
NUM_GPUS="${3:-${NUM_GPUS:-1}}"
TASK_ID="${TASK_ID:-Tracking-Flat-WalkerC1-Simple-v0}"
RUN_PREFIX="${RUN_PREFIX:-walker_c1}"
RESUME="${RESUME:-false}"
RESUME_RUN="${4:-${RESUME_RUN:-0719_230924_walker_c1_full}}"
RESUME_CHECKPOINT="${5:-${RESUME_CHECKPOINT:-model_1400.pt}}"
LOAD_CKPT="${LOAD_CKPT:-policy/c1/wbt_model_26600.pt}"
EPISODE_LENGTH_S="${EPISODE_LENGTH_S:-}"
EPISODE_LENGTH_CAP_S="${EPISODE_LENGTH_CAP_S:-}"
EPISODE_LENGTH_SCALE="${EPISODE_LENGTH_SCALE:-1.0}"
CURRICULUM_INITIAL_STAGE="${CURRICULUM_INITIAL_STAGE:-0}"
...
echo "========================================"
echo " Walker C1 Training — $MODE (Pro Strategy)"
echo "========================================"
echo " Task       : $TASK_ID"
